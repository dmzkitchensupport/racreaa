...
async function sendReport({ auditId, body, serverTs, tenantId }) {
  ...
  const itemsHtml = (body.items || []).map(item => {
    ...
    const critRows = Object.entries({
      ...
      return `<tr>
        ...
        <td style="padding:3px 8px"><div style="background:#E8E4DA;border-radius:2px;height:7px"><div style="background:${c};height:7px;border-radius:2px;width:${pct}%"></div></div></td>
        <td style="padding:3px 8px;font-size:12px;font-weight:700;color:${c};width:40px;text-align:right">${v}/10</td>
      </tr>`;
    }).join('');
    ...
    return `<div style="background:#fff;border:1px solid #E0DDD4;border-left:4px solid ${niv.color};margin-bottom:12px;border-radius:2px">
      <div style="padding:10px 14px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #F0EDE6">
        <div>
          <span style="font-size:10px;color:#9A9890;font-weight:700">${String(item.num).padStart(2,'0')}</span>
          <span style="font-size:14px;font-weight:700;color:#2C2A24;margin-left:8px">${sanitize(item.nombre||'Sin nombre')}</span>
          <span style="font-size:10px;color:#9A9890;margin-left:6px">${sanitize(item.categoria||'')}</span>
        </div>
        <div style="background:${niv.color};padding:6px 14px;border-radius:2px;text-align:center;min-width:60px">
          <div style="font-size:20px;font-weight:900;color:#fff;line-height:1">${item.score||0}</div>
          <div style="font-size:8px;color:rgba(255,255,255,.85);text-transform:uppercase;letter-spacing:1px">${niv.label}</div>
        </div>
      </div>
      ...
      ${item.observaciones ? `<div style="padding:8px 14px;border-bottom:1px solid #F0EDE6"><span style="font-size:9px;color:#8A6C1E;text-transform:uppercase;letter-spacing:1px">Observaciones: </span><span style="font-size:12px;color:#5C5A54">${sanitize(item.observaciones,500)}</span></div>` : ''}
      ${fotosTag}
    </div>`;
  }).join('');
  ...
  const firmaHtml = body.signatureImage
    ? `<div style="padding:16px 32px;border-bottom:1px solid #E0DDD4">
        ...
        <div style="font-size:11px;color:#5C5A54;margin-top:4px;font-weight:600">${sanitize(body.auditorFirma||body.auditor||'—')}</div>
      </div>` : '';
  ...
  <div style="max-width:620px;margin:0 auto;background:#fff;border-top:4px solid #C4922A;box-shadow:0 2px 8px rgba(0,0,0,.1)">
    ...
    <div style="padding:20px 32px;border-bottom:1px solid #E0DDD4">
      <table style="width:100%;border-collapse:collapse">
        <tr><td style="padding:4px 0;font-size:10px;color:#9A9890;text-transform:uppercase;letter-spacing:1px;width:130px">ID</td><td style="font-size:11px;font-family:monospace">${auditId}</td></tr>
        <tr><td style="padding:4px 0;font-size:10px;color:#9A9890;text-transform:uppercase;letter-spacing:1px">Establecimiento</td><td style="font-size:14px;font-weight:700">${sanitize(body.establecimiento)||'—'}</td></tr>
    ...
    <div style="padding:24px 32px;text-align:center;border-bottom:1px solid #E0DDD4;background:#FAFAF7">
      <div style="display:inline-block;background:${nivel.color};padding:20px 48px;border-radius:4px">
        <div style="font-size:56px;font-weight:900;color:#fff;line-height:1">${body.globalScore||0}</div>
        <div style="font-size:11px;font-weight:700;letter-spacing:3px;color:rgba(255,255,255,.85);text-transform:uppercase;margin-top:6px">${nivel.label}</div>
      </div>
      <div style="font-size:12px;color:#9A9890;margin-top:12px">${(body.items||[]).length} platillo${(body.items||[]).length!==1?'s':''} evaluado${(body.items||[]).length!==1?'s':''} · ${attachments.length > 0 ? `📎 ${attachments.length} foto${attachments.length>1?'s':''} adjunta${attachments.length>1?'s':''}` : 'Sin fotos'}</div>
    </div>
    ...
    ${body.conclusion ? `<div style="padding:20px 32px;border-bottom:1px solid #E0DDD4"><div style="font-size:9px;font-weight:700;letter-spacing:3px;color:#8A6C1E;text-transform:uppercase;margin-bottom:8px">Dictamen</div><p style="font-size:13px;color:#2C2A24;line-height:1.8;margin:0;font-style:italic">"${sanitize(body.conclusion,2000)}"</p></div>` : ''}
    ${firmaHtml}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // WhatsApp — enlace con resumen (se abre desde el servidor solo como log)
    const nivel_wa = getNivel(body.globalScore||0);
    const wa_msg = [
      `📋 *REPORTE DMZ AUDIT*`,
      `🏢 *${body.establecimiento||'—'}* · ${body.globalScore||0}/100 *${nivel_wa.label.toUpperCase()}*`,
      `👤 Auditor: ${body.auditorFirma||body.auditor||'—'} · 📅 ${body.fecha||'—'} ${body.servicio||''}`,
      (body.items||[]).length > 0 ? `📊 ${(body.items||[]).length} platillo${(body.items||[]).length!==1?'s':''} evaluado${(body.items||[]).length!==1?'s':''}` : '',
      body.conclusion ? `💬 _${body.conclusion.slice(0,120)}${body.conclusion.length>120?'…':''}_` : '',
      `🆔 \`${auditId}\``,
      `_DMZ Kitchen Support_`
    ].filter(Boolean).join('\n');

    console.log('[WA_LINK]', 'https://wa.me/522223623135?text=' + encodeURIComponent(wa_msg));
    ...
    return res.status(200).json({
      ...
      email_sent,
      message: `Evaluación registrada. ${totalFotos} foto${totalFotos!==1?'s':''} guardada${totalFotos!==1?'s':''}. ${email_sent ? '📧 Reporte enviado.' : 'Email pendiente.'}`
    });
